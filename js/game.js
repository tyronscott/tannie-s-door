...
// if the date is not yet Feb 7 12:00 AM 2024
	...
	constructor(config, isIntro) {
		...
		self.draw = function () {
			...
			if (self.isIntro) {
				...
				// draw the time left until birthday
				if (!isBirthday) {
					var timeLeft = tannie.getTime() - now;
					var days = Math.floor(timeLeft / (1000 * 60 * 60 * 24));
					var hours = Math.floor((timeLeft % (1000 * 60 * 60 * 24)) / (1000 * 60 * 60));
					var minutes = Math.floor((timeLeft % (1000 * 60 * 60)) / (1000 * 60));
					var seconds = Math.floor((timeLeft % (1000 * 60)) / 1000);
					ctx.fillText("Key will spawn in  " + days + "d " + hours + "h " + minutes + "m " + seconds + "s", cx, cy + 200);
				}
				ctx.restore();
			}
	...
	constructor(config, level) {
		...
		self.update = function () {
			...
			if (Key.left || Key.right || Key.up || Key.down) {
				if (self.level.clock.paused) {
					self.level.clock.play();
				}
...
window.INTRO_LEVEL = {
	...
	key: { x: cx, y: isBirthday ? cy + 125 : -20000 },
	circles: [
